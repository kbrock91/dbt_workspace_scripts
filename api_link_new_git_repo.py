...
# Configuration
API_BASE_URL = 'https://cloud.getdbt.com/api/v3' #update to match your dbt Cloud account API Base URL
...
def get_current_project_info(dbt_cloud_project_id):
    ...
    if response.status_code in (200, 201):
        print(f"✅ Project fetched successfully. name: {response.json()['data']['name']}, repo_id: {response.json()['data'].get('repository_id')}, repo_name:  {response.json()['data']['repository'].get('github_repo')}")
        return response.json()['data']['name'], response.json()['data'].get('repository_id'), response.json()['data']['repository'].get('github_repo')
    else:
...
def create_repository(new_github_repo):
    payload = {
        ...
        'pull_request_url_template': f'https://github.com/{new_github_repo}/compare/{{source}}...{{destination}}'
    }
...
if __name__ == "__main__":
    try:
        project_name, current_repo_id, current_github_repo = get_current_project_info(DBT_CLOUD_PROJECT_ID)
        delete_repo(current_repo_id)
        new_github_repo = current_github_repo.replace(current_github_org, new_github_org)
        repo_id = create_repository(new_github_repo)
